...
Ключевое ограничение
 Реальная авторизация через парсинг реестра СРО отложена до v1.1. В MVP используется эмуляция через локальный файл local_reestr.txt.
...
Авторизация через реестр
 ❌ v1.1
 Парсинг https://www.sronoso.ru/reestr/
...
Supabase настроен (URL и KEY подключены).
Технический стек: React 18 + TypeScript + Vite 5.
